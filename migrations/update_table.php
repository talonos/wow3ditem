...
class update_table extends \phpbb\db\migration\migration
{
	...
	{
		return array(
		 array('custom', array(array($this, 'install_bbcode_for_3ditem'))),
		);
	}
	...
	public function install_bbcode_for_3ditem()
	{
		...
		$bbcode_name = '3ditem';
		$bbcode_array = array(
		 'bbcode_match'		=> '[3ditem]{NUMBER}[/3ditem]',
		 'bbcode_tpl'		=> '
...
r(function(){
      ...
      $("#model-{NUMBER}").spritespin({
        ...
        height    : 280,
        frames    : 24,
        image     : "http://media.blizzard.com/wow/renders/items/item{NUMBER}.jpg",
      animate   : false,
        loop      : true,
      });
  ...
  <div>
  <div align="center" width="280px" height="280px" ><div id="item-{NUMBER}"><a href="http://www.wowhead.com/item={NUMBER}"></a></div>
    <div class="model-viewer" id="model-{NUMBER}"></div>
   
</div>
</div>

',
		 'bbcode_helpline'	=> '[3ditem]ITEM ID NUMBER[/3ditem]',
	...
	{
		$sql = 'DELETE FROM ' . BBCODES_TABLE . ' WHERE bbcode_tag = \'3ditem\'';
		$this->db->sql_query($sql);
	}
}
